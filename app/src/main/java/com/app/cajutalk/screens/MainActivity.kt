...
fun CajuTalkApp() {
    ...
    val authViewModel: AuthViewModel = viewModel() // Instantiate AuthViewModel here

    FocusClearContainer{
        NavHost(
            navController = navController,
            startDestination = if (authViewModel.isLoggedIn()) "salas" else "cadastro", // Check if logged in
            enterTransition = { fadeIn(animationSpec = tween(300)) },
            exitTransition = { fadeOut(animationSpec = tween(300)) },
            popEnterTransition = { fadeIn(animationSpec = tween(300)) },
            popExitTransition = { fadeOut(animationSpec = tween(300)) }
        ) {
            composable("login") { LoginScreen(navController, dataViewModel) } // Pass dataViewModel
            composable("cadastro") { CadastroScreen(navController, dataViewModel) } // Pass dataViewModel
            ...
            composable("user-profile") { UserProfileScreen(navController, dataViewModel) } // Pass dataViewModel
